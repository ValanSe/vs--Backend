...
(
    `id`    VARCHAR(255) COMMENT '네이버 사용자 ID',
...
(
    `id`    VARCHAR(255) COMMENT '구글 사용자 고유 ID',
    `email` VARCHAR(100) COMMENT '이메일 주소',
    `name`  VARCHAR(100) COMMENT '사용자 이름',
    PRIMARY KEY (`id`)
) ENGINE = InnoDB
...
(
    `id`   VARCHAR(255) COMMENT '카카오 사용자 고유 ID',
    `name` VARCHAR(100) COMMENT '사용자 이름',
    PRIMARY KEY (`id`)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
...
CREATE TABLE `user`
(
    `user_id`        INT AUTO_INCREMENT COMMENT '사용자 식별자',
...
CREATE TABLE `quiz`
(
    `quiz_id`        INT AUTO_INCREMENT COMMENT '질문 식별자',
...
(
    ...
    `selected_option` VARCHAR(255) NOT NULL COMMENT '선택된 옵션',
...
CREATE TABLE `notice`
(
    notice_id  INT AUTO_INCREMENT COMMENT '공지사항 식별자',
...
(
    comment_id     INT AUTO_INCREMENT COMMENT '댓글 식별자',
...
(
    ...
    recommend_score    INT AS (answer_count +
                               total_preference +
                               comment_count * 2 +
                               registration_count * 2) STORED COMMENT '해당 카테고리 추천 점수',
