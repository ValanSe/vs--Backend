...
public interface CategoryService {
    ...
    void addQuestionToCategory(Category category, Question question); // 카테고리에 질문 추가
    void removeQuestionFromCategory(Category category, Question question); // 카테고리에서 질문 제거

    // 데이터베이스에서 질문이 갱신될 시 카테고리에 있는 질문도 갱신
    void updateQuestionsInCategory(Category category, Question updatedQuestion);
